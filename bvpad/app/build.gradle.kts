...
android {
    ...
    defaultConfig {
        ...
        ndk {
            // release 只打 arm64-v8a；debug 额外打 x86_64 便于 x86 模拟器调试
            abiFilters += if (gradle.startParameter.taskNames.any {
                    it.contains("Release", true) || it.contains("R8Test", true) || it.contains("Alpha", true)
                }) {
                listOf("arm64-v8a")
            } else {
    ...
    packaging {
        ...
        if (gradle.startParameter.taskNames.find { it.startsWith("assembleLite") } != null) {
            jniLibs {
                ...
                vlcLibs.forEach { vlcLibName -> abis.forEach { abi -> this.excludes.add("lib/$abi/$vlcLibName.so") } }
            }
        }
    }
